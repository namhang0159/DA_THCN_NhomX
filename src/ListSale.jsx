...
//         <button
//           className=" text-black rounded-2xl p-4 items-center justify-center
//         "
//           onClick={() => {
//             navigate("/pageall");
//           }}
...
//               <div
//                 className="flex flex-col items-center bg-white p-4 h-full rounded-2xl shadow-md hover:shadow-xl transition duration-300"
...
export const ListSale = ({ title, data }) => {
  const navigate = useNavigate();
  const [percentage, setPercentage] = useState(33.33); // mặc định desktop

  // bắt sự kiện resize để set % hiển thị
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) {
        setPercentage(100); // mobile 1 item
      } else if (window.innerWidth < 1024) {
        setPercentage(50); // tablet 2 item
      } else {
        setPercentage(33.33); // desktop 3 item
      }
    };
    handleResize();
